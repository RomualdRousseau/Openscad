...
{
    // top base
    if(!solid_holes) color("DarkSlateGray") translate([0, 0, 0.4]) difference()
    {
    ...
    // bottom base
    if(!solid_holes) color("DarkSlateGray") translate([0, 0, -1.7])
    {
        difference()
        {
            union()
    ...
    if(!solid_holes) translate([0, 0, -1.1]) rotate([angle_y, 0, 0])
    {
        // gimbal y
        ...
        translate([0, 0, 0.2]) 
        {
            ...
            rotate([0, angle_x, 0])
            {
                ...
                color("silver") translate([0, 0, 1.3]) rotate_extrude() polygon(points = [[0, 0], [0, 3.3], [0.3, 3.5], [0.4, 3.2], [0.4, 2], [0.2, 1], [0.2, 0.6], [0.4, 0.6], [0.4, 0]]);
            }
        }
    }
    ...
    if(solid_holes)
    {
        translate([0, 0, 0.4]) cylinder(h = 1.8, d = 4.9, center = true);
        translate([-1.1, -2.65, -1]) cylinder(d = 0.2, h = thickness + 1.1);
        translate([1.1, -2.65, -1]) cylinder(d = 0.2, h = thickness + 1.1);
        translate([-1.1, 2.65, -1]) cylinder(d = 0.2, h = thickness + 1.1);
        translate([1.1, 2.65, -1]) cylinder(d = 0.2, h = thickness + 1.1);
    }
